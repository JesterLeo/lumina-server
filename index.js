...
// ── AI PROXY (fixes browser CORS — all AI calls go through here) ──
app.post("/api/ai", async (req, res) => {
  const key = process.env.ANTHROPIC_API_KEY;
  if (!key) return res.status(500).json({ error: "ANTHROPIC_API_KEY not set. Add it as an environment variable in Render." });
  try {
